...
bool DXCBPlatformWindowInterface::eventFilterForXcb(QObject *obj, QEvent *event)
{
    D_D(DXCBPlatformWindowInterface);
    if (obj == d->m_window) {
        if (event->type() == QEvent::DynamicPropertyChange) {
            ...
            if (e->propertyName() == _windowRadius) {
                Q_EMIT d->m_platformHandle->windowRadiusChanged();
            } else if (e->propertyName() == _borderWidth) {
                Q_EMIT d->m_platformHandle->borderWidthChanged();
            } else if (e->propertyName() == _borderColor) {
                Q_EMIT d->m_platformHandle->borderColorChanged();
            } else if (e->propertyName() == _shadowRadius) {
                Q_EMIT d->m_platformHandle->shadowRadiusChanged();
            } else if (e->propertyName() == _shadowOffset) {
                Q_EMIT d->m_platformHandle->shadowOffsetChanged();
            } else if (e->propertyName() == _shadowColor) {
                Q_EMIT d->m_platformHandle->shadowColorChanged();
            } else if (e->propertyName() == _clipPath) {
                Q_EMIT d->m_platformHandle->clipPathChanged();
            } else if (e->propertyName() == _frameMask) {
                Q_EMIT d->m_platformHandle->frameMaskChanged();
            } else if (e->propertyName() == _frameMargins) {
                Q_EMIT d->m_platformHandle->frameMarginsChanged();
            } else if (e->propertyName() == _translucentBackground) {
                Q_EMIT d->m_platformHandle->translucentBackgroundChanged();
            } else if (e->propertyName() == _enableSystemResize) {
                Q_EMIT d->m_platformHandle->enableSystemResizeChanged();
            } else if (e->propertyName() == _enableSystemMove) {
                Q_EMIT d->m_platformHandle->enableSystemMoveChanged();
            } else if (e->propertyName() == _enableBlurWindow) {
                Q_EMIT d->m_platformHandle->enableBlurWindowChanged();
            } else if (e->propertyName() == _autoInputMaskByClipPath) {
                Q_EMIT d->m_platformHandle->autoInputMaskByClipPathChanged();
            }
        }
    }
